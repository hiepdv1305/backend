...
module.exports.create_balance_fluctuation = async (amount, description, userId) => {
    const balance_fluctuation_table_ = client.db(db).collection(balance_fluctuation_table)
    ...
    return user_table_.updateOne({ _id: new ObjectId(userId) }, { $set: { balance: user_data.balance } }).then(res => {
        return balance_fluctuation_table_.insertOne({
            userId: userId,
            amount: amount,
...
module.exports.getBalanceFluctuation = async (event, context, callback) => {
    ...
    balance_fluctuation_table_.findOne({ _id: new ObjectId(id) }).then(res => {
        if (user.role != "admin" && user._id != res.userId) return response("", "no permision", 500)
...
module.exports.getAllBalanceFluctuation = async (event, context, callback) => {
    ...
    const balance_fluctuation_table_ = client.db(db).collection(balance_fluctuation_table)
    if (user.role != "admin") {
        balance_fluctuation_table_.find({ userId: user._id }).toArray().then(res => {
            return response(res, "success", 200)
        })
    } else {
        balance_fluctuation_table_.find({}).toArray().then(res => {
            return response(res, "success", 200)
        })
    }
}