...
module.exports.register = async (event, context, callback) => {
    ...
    return user_table_.findOne(params).then((res) => {
        console.log(res)
        if (res) {
            console.log(1)
            return response("", "username already exists", 400);
        } else {
            return user_table_.insertOne(data)
                .then(() => {
...
module.exports.login = async (event, context, callback) => {
    ...
    return user_table_.findOne(params).then(async (res) => {
        console.log(res)
        if (res && bcrypt.compareSync(data.password, res.password)) {
            const accessToken = await jwtHelper.generateToken(res, config.accessTokenSecret, config.accessTokenLife)
            const refreshToken = await jwtHelper.generateToken(res, config.refreshTokenSecret, config.refreshTokenLife)
